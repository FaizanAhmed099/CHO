...
(function () {
  ...
  function setHostClickListener(host, handler) {
    if (__removeHostClickListener) {
      try { __removeHostClickListener(); } catch (_) {}
      __removeHostClickListener = null;
    }
    host.addEventListener('click', handler);
    __removeHostClickListener = () => host.removeEventListener('click', handler);
  }
        ...
        <table class="table table-striped align-middle">
          ...
          <tbody>
            ${items
              .map((d) => `
                <tr data-id="${d._id}">
                  <td>${d.image ? `<img src="${imgUrl(d.image)}" class="thumb">` : ''}</td>
                  <td>${d.name || ''}</td>
                  <td>${d.role || ''}</td>
  ...
  function renderDirectorCreateForm() {
    ...
    function clearImageError() { showImageError(''); }
    ...
    dz.addEventListener('drop', (e) => {
      ...
      if (!f) return;
      if (!f.type?.startsWith('image/')) return showImageError('Image must be an image file');
      fileInput.files = e.dataTransfer.files; // ensure FormData picks it up
      setPreview(f);
    });
    fileInput.addEventListener('change', (e) => {
      const f = e.target.files?.[0];
      if (!f) { setPreview(null); showImageError('Image is required'); return; }
      if (!f.type?.startsWith('image/')) { setPreview(null); showImageError('Image must be an image file'); return; }
      setPreview(f);
    });

    // Slug auto-generate from name (unless user edits slug)
    let slugTouched = false;
    slugInput.addEventListener('input', () => { slugTouched = true; clearFieldError(slugInput); });
    function slugify(s) {
    ...
    nameInput.addEventListener('input', () => {
      ...
      if (val) clearFieldError(nameInput); else setFieldError(nameInput, 'Name is required');
      if (!slugTouched) slugInput.value = slugify(val);
    });
    roleInput.addEventListener('input', () => {
      const val = roleInput.value.trim();
      if (val) clearFieldError(roleInput); else setFieldError(roleInput, 'Role is required');
    });
    ...
    form.addEventListener('submit', async (e) => {
      ...
      let hasError = false;
      if (!nameVal) { setFieldError(nameInput, 'Name is required'); hasError = true; }
      if (!roleVal) { setFieldError(roleInput, 'Role is required'); hasError = true; }
      if (!imgFile) { showImageError('Image is required'); hasError = true; }
      else if (!imgFile.type?.startsWith('image/')) { showImageError('Image must be an image file'); hasError = true; }

      if (hasError) {
        toast('Please correct the highlighted errors', 'error');
        (document.querySelector('.is-invalid') || (!imgFile ? dz : null))?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
        return;
      }
      ...
      try {
        ...
        const details = err?.body?.errors || err?.body?.message || 'Create failed';
  ...
  async function renderDirectorEditForm(id) {
      ...
      <form id="dirEditForm" class="card p-3 shadow-sm" novalidate>
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <div class="mt-2">
              <div id="dirEditImagePreviewWrap" class="border rounded overflow-hidden position-relative" style="width:120px; height:120px; max-width:100%; ${data.image ? '' : 'display:none;'}">
                <img id="dirEditImagePreview" alt="image-preview" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" src="${data.image ? imgUrl(data.image) : ''}">
              </div>
            </div>
          ...
          <div class="col-md-6">
            ...
            <input name="slug" class="form-control" value="${data.slug || ''}" />
          </div>
          <div class="col-12">
          ...
          <div class="col-12">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="isActiveDir" name="isActive" ${data.isActive !== false ? 'checked' : ''} />
              <label class="form-check-label" for="isActiveDir">Active</label>
            </div>
          </div>
        </div>
    ...
    nameInput.addEventListener('input', () => {
      ...
      if (v) clearFieldError(nameInput); else setFieldError(nameInput, 'Name is required');
    });
    roleInput.addEventListener('input', () => {
      const v = roleInput.value.trim();
      if (v) clearFieldError(roleInput); else setFieldError(roleInput, 'Role is required');
    });
    ...
    fileInput.addEventListener('change', (e) => {
      const f = e.target.files?.[0];
      if (!f) { clearImageError(); return; }
      if (!f.type?.startsWith('image/')) { showImageError('Image must be an image file'); return; }
      setPreview(f);
    });
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!nameVal) { setFieldError(nameInput, 'Name is required'); hasError = true; }
      if (!roleVal) { setFieldError(roleInput, 'Role is required'); hasError = true; }
      if (imgFile && !imgFile.type?.startsWith('image/')) { showImageError('Image must be an image file'); hasError = true; }
      if (hasError) {
        toast('Please correct the highlighted errors', 'error');
        return;
      }
      ...
      try {
        if (imgFile) {
          ...
          fd.set('slug', slugVal || nameVal.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, ''));
          ...
          const payload = {
            ...
            slug: slugVal || nameVal.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, ''),
            isActive,
          };
          await apiJson(`/api/directors/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
        }
        toast('Director updated', 'success');
        renderDirectorsTab();
      } catch (err) {
        const details = err?.body?.errors || err?.body?.message || 'Update failed';
        toast(Array.isArray(details) ? details.join(', ') : details, 'error');
      }
    });
  }
        ...
        <table class="table table-striped align-middle">
          ...
          <tbody>
            ${items
              .map((p) => `
                <tr data-id="${p._id}">
                  <td>${p.image ? `<img src="${imgUrl(p.image)}" class="thumb">` : ''}</td>
                  <td>${p.title || ''}</td>
                  <td>${p.category || ''}</td>
                  <td>${p.status || ''}</td>
  ...
  async function renderProjectEditForm(id) {
    ...
    $('#tabContent').innerHTML = `
      ...
      <form id="projEditForm" class="card p-3 shadow-sm">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            <label class="form-label">Category</label>
            <input name="category" class="form-control" value="${data.category || ''}" />
          </div>
          <div class="col-md-3">
            <label class="form-label">Status</label>
            <input name="status" class="form-control" value="${data.status || ''}" />
          </div>
          <div class="col-md-6">
            <label class="form-label">Replace Image (optional)</label>
            <input name="image" type="file" accept="image/*" class="form-control" />
            ${data.image ? `<div class='mt-2'><img src='${imgUrl(data.image)}' class='thumb'></div>` : ''}
          </div>
          <div class="col-md-6">
            <label class="form-label">Slug</label>
            <input name="slug" class="form-control" value="${data.slug || ''}" />
          ...
          <div class="col-12">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="isActiveProj" name="isActive" ${data.isActive !== false ? 'checked' : ''} />
              <label class="form-check-label" for="isActiveProj">Active</label>
            </div>
          </div>
        </div>
    ...
    $('#projEditForm').addEventListener('submit', async (e) => {
      e.preventDefault();
      const fd = new FormData(e.currentTarget);
      const hasNewImage = fd.get('image') instanceof File && fd.get('image').name;
          ...
          <div class="col-6 col-md-4 col-lg-3" data-image-id="${it._id}">
            <div class="card h-100">
              <div class="position-relative">
                <img src="${imgUrl(it.url)}" class="card-img-top" style="height: 200px; object-fit: cover; border-top-left-radius: .5rem; border-top-right-radius: .5rem;">
              </div>
              <div class="d-flex justify-content-center align-items-center gap-3 py-2 px-3 bg-light rounded-bottom">
                <div class="form-check form-switch m-0" title="Active">
                  <input class="form-check-input border-primary" style="transform: scale(1.2); cursor: pointer;" type="checkbox" data-action="bulk-toggle-active" ${it.isActive !== false ? 'checked' : ''}>
                </div>
                ...
                <h2 class="accordion-header" id="heading-${collapseId}">
                  <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}" aria-expanded="false" aria-controls="${collapseId}">
                    <div class="d-flex w-100 justify-content-between align-items-center">
                      <span class="me-3">Batch ${i + 1} • ${count} images • ${created}</span>
                      ...
                      ${(b.images || [])
                        .map((p) => `
                          <div class="col-6 col-md-3 col-lg-2">
                            <img src="${imgUrl(p)}" class="img-fluid rounded" style="height: 100px; object-fit: cover; width: 100%;"/>
                          </div>
                        `)
                        .join('')}
                    </div>
  ...
  function bindGalleryEvents(host) {
    const handler = async (e) => {
      ...
      if (action === 'bulk-delete-image') {
        const id = btn.closest('[data-image-id]')?.getAttribute('data-image-id');
  ...
  function showBulkUploadForm() {
    ...
    function renderPreviews() {
      if (!selectedFiles.length) {
        previewGrid.innerHTML = '<div class="text-muted small">No images selected yet.</div>';
        return;
      }
    ...
    $('#bulkUploadForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const details = err?.body?.errors || err?.body?.message || 'Bulk upload failed';
        toast(Array.isArray(details) ? details.join(', ') : details, 'error');
      }
    });
  }
        ...
        <table class="table table-striped align-middle">
          ...
          <tbody>
            ${items
              .map((a) => `
                <tr data-id="${a._id}">
                  <td>${a.image ? `<img src="${imgUrl(a.image)}" class="thumb">` : ''}</td>
                  <td>${a.title || ''}</td>
                  <td>${a.year || ''}</td>
                  <td>${a.monthName || ''}</td>
  ...
  function monthOptions(selected) {
    ...
    return months
      .map((m) => `<option value="${m}" ${selected === m ? 'selected' : ''}>${m}</option>`) 
      .join('');
  }
  ...
  async function renderAwardEditForm(id) {
    ...
    $('#tabContent').innerHTML = `
      ...
      <form id="awardEditForm" class="card p-3 shadow-sm">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            <label class="form-label">Year</label>
            <input name="year" type="number" min="1900" max="3000" class="form-control" value="${data.year || ''}" />
          </div>
          <div class="col-md-3">
            <label class="form-label">Month</label>
          ...
          <div class="col-12">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="isActiveAward" name="isActive" ${data.isActive !== false ? 'checked' : ''} />
              <label class="form-check-label" for="isActiveAward">Active</label>
            </div>
          </div>
        </div>
    ...
    $('#awardEditForm').addEventListener('submit', async (e) => {
      ...
      try {
        if (hasNewImage) {
          fd.set('isActive', isActive ? 'true' : 'false');
          await apiForm(`/api/awards-certificates/${id}`, fd, { method: 'PUT' });
        } else {
          const payload = {
            title: fd.get('title') || undefined,
          ...
          await apiJson(`/api/awards-certificates/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
        }
        ...
        <table class="table table-striped align-middle">
          ...
          <tbody>
            ${items
              .map((c) => `
                <tr data-id="${c._id}">
                  <td>${c.logo ? `<img src="${imgUrl(c.logo)}" class="thumb">` : ''}</td>
                  <td>${c.name || ''}</td>
  ...
  function renderClientCreateForm() {
    ...
    if (dzE) {
      ...
      dzE.addEventListener('drop', (e) => {
        ...
        if (!file) return;
        if (!file.type?.startsWith('image/')) return showEditLogoError('Logo must be an image file');
    ...
    if (descInput) descInput.addEventListener('input', () => {
      ...
      if (val) clearFieldError(descInput); else setFieldError(descInput, 'Description is required');
    });
    ...
    $('#clientForm').addEventListener('submit', async (e) => {
      ...
      if (!nameVal) { setFieldError(nameInput, 'Name is required'); hasError = true; }
      if (!descVal) { setFieldError(descInput, 'Description is required'); hasError = true; }
      if (!logoFile) { showLogoError('Logo image is required'); hasError = true; }
      else if (!logoFile.type?.startsWith('image/')) { showLogoError('Logo must be an image file'); hasError = true; }

      if (hasError) {
        toast('Please correct the highlighted errors', 'error');
        const firstError = document.querySelector('.is-invalid') || (!logoFile ? dz : null);
      ...
      <form id="clientEditForm" class="card p-3 shadow-sm">
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <div class="mt-2">
              <div id="logoEditPreviewWrap" class="border rounded overflow-hidden position-relative" style="width:138px; height:137px; ${data.logo ? '' : 'display:none;'}">
                <img id="logoEditPreview" alt="logo-preview" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" src="${data.logo ? imgUrl(data.logo) : ''}">
              </div>
            </div>
          ...
          <div class="col-12">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="isActive" name="isActive" ${data.isActive !== false ? 'checked' : ''} />
              <label class="form-check-label" for="isActive">Active</label>
            </div>
          </div>
        </div>
  ...
  function init() {
    ...
    apiJson('/api/admin/users')
      ...
      .catch(() => {/* handleUnauthorized already redirected */});
  }
